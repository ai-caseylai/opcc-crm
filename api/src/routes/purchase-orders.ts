...
po.patch('/:id/status', zValidator('json', z.object({ status: z.string() })), async (c) => {
  ...
  const existing = await db.prepare('SELECT id FROM purchase_orders WHERE id = ? AND user_id = ?').bind(id, user.id).first();
...
po.delete('/:id', async (c) => {
  const user = c.get('user');
  const id = c.req.param('id');
  const existing = await c.env.DB.prepare('SELECT id FROM purchase_orders WHERE id = ? AND user_id = ?').bind(id, user.id).first();
  if (!existing) return c.json({ error: 'Purchase order not found' }, 404);
